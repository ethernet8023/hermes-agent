...
export const zh = defineLocale({
  ...
  settings: {
    ...
    sandbox: {
      heading: 'Windows 沙箱',
      description:
        '受支持的终端和文件工具在 Windows 进程容器（MXC）中运行。未隔离的工具会被拒绝；主机推理和 Hermes 固定的内部管理功能仍是容器外的可信服务。',
      statusAvailable: '可用',
      statusDegraded: '可用（受限）',
      statusUnavailable: '不可用',
      statusUnknown: '尚未确认保护状态。请重新检查后端，或关闭沙箱以恢复操作。',
      modelOutputBlocked: 'MXC 已阻止预览：助手内容不能在此读取主机文件、加载远程内容或执行脚本。',
      modelOutputUnconfirmed: '确认此内容的所属来源和沙箱策略之前，预览保持禁用。',
      toggleLabel: '沙箱化终端和文件工具',
      toggleDescription: '适用于此配置的所有对话。策略更改完成前会停止原有的主机执行；下一条受支持的命令使用新策略。',
      shellNote: '首次开启时会下载托管的 shell，除非你已配置自定义 shell。',
      workspaceRule: '每个对话在用户选择的项目文件夹或默认 Hermes 工作区中写入。下方额外文件夹的授权包含所有子目录。运行时工具、私有临时空间和受支持的粘贴附件有范围有限的内部访问权限。',
      foldersTitle: '其他文件夹',
      isolationRule: '受支持的命令无法使用主机剪贴板、注册表或其他进程。仅传入选定的运行时环境变量，不传入主机凭据。',
      foldersEmpty: '没有额外的用户文件夹授权。',
      addReadOnly: '添加只读文件夹',
      ...
      modeReadWrite: '读写',
      networkLabel: '允许网络访问',
      networkDescription: '控制沙箱出站网络和网页搜索/提取。即使开启网络，浏览器自动化、桌面控制、MCP、连接器、生成工具和 execute_code 仍被拒绝。主机推理和可信内部管理不受此开关控制。',
  ...
  composer: {
    ...
    sandbox: {
      heading: '沙箱（MXC）',
      titleOn: '关闭此配置所有对话的沙箱',
      titleOff: '开启此配置所有对话的沙箱',
      unknown: '尚未确认',
      unavailable: '不可用',
      on: '开',
      off: '关',
      descriptionOn: folder =>
        `此配置使用 MXC 运行受支持的终端和文件工具。此对话在 ${folder} 中工作，也可访问设置中额外授权的文件夹及其子目录。`,
      descriptionOff: '开启沙箱，为此配置所有对话中受支持的终端和文件工具提供隔离。',
      networkOn: '已允许网络访问。',
      networkOff: '网络访问已关闭。',
      isolated: '开启网络也不会放行未隔离的工具。主机推理和 Hermes 固定的内部管理仍是可信服务。',
      openSettings: '打开沙箱设置',
      turnOnFailed: '无法开启沙箱',
  ...
  assistant: {
    ...
    tool: {
      ...
      sandboxRecursiveScope: path => `包含 ${path} 中的所有文件和子文件夹。此授权适用于此配置的所有对话。`,
