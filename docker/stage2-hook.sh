...
# The Chromium discovery block that used to live here is gone. It existed
# because the image baked its browser with `npx playwright install`, which
# recorded no fact: agent-browser could not find the binary in that layout
# (#15697), so the hook located it at boot and exported
# AGENT_BROWSER_EXECUTABLE_PATH into s6's container_environment.
#
# The image provisions the pinned browser now, so a fact records it and
# installation/env.py exports PLAYWRIGHT_BROWSERS_PATH from that fact for
# every subprocess. This also closes a hole the hook could not: it wrote
# to /run/s6/container_environment, which `with-contenv` gives to the
# SUPERVISED services only, so a `docker exec` shell never saw the
# variable and resolved no browser at all. A fact is read from disk by
# whatever process asks.
#
# An operator who sets AGENT_BROWSER_EXECUTABLE_PATH still wins: it is
# the first rung of installation.browser.engine_path.
