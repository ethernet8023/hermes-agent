...
def _resolve_verify(
    *,
    ...
    """TLS verify for auth-portal clients — the one resolver, given this
    call's explicit inputs (per-connection ``tls`` state or an argument)."""
    ...
    return resolve_httpx_verify(
        ca_bundle=ca_bundle or tls_state.get("ca_bundle"),
...
def _login_nous(args, pconfig: ProviderConfig) -> None:
    ...
    # --ca-bundle only: trust otherwise comes from the OS certificate store,
    # so there is no env var to consult.
