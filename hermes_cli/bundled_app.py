...
The shape authority is the build stamp (``payload: bundled``), never a
filesystem probe: a probe answers "is this artifact intact?", not "which
shape am I?". Callers gate on the stamp, then ask here WHERE the app is.
A stamp that says bundled over a tree that is not one is damage, and
:func:`resolve_bundle_layout` says so instead of degrading to a checkout.
