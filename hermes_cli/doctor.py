...
def check_certificates(should_fix: bool = False, issues: "list | None" = None) -> None:
    """Report where TLS trust comes from.

    Trust is the OS store via truststore, so there is no bundle to repair:
    a machine whose certificate store is broken is broken for every app on
    it, and fixing that is the OS's job, not ours. ``should_fix`` is
    accepted for signature parity with the other checks.
    """
    try:
    ...
    if install_truststore():
        check_ok("TLS trust: operating system certificate store")
        return

    check_fail(
        "TLS trust: OpenSSL defaults (truststore unavailable)",
        "Certificates trusted only by the OS store — a corporate root, for "
        "instance — will not verify.",
    )
    if issues is not None:
        issues.append(
            "truststore could not load; reinstall it: "
            f"{sys.executable} -m pip install --force-reinstall truststore"
        )
