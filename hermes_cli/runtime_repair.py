...
artifact (``venv_sync`` reports ``sealed`` and stops).

uv itself is NOT this module's business: the pinned uv is located through
``installation.uv`` (registry facts + provisioner), like every other
managed tool.  This module was previously ``hermes_cli.managed_uv`` and
carried its own uv acquisition; that half predated the tool store and is
retired.
"""
...
def managed_python_install_dir(project_root: Path | None = None) -> Path:
    ...
    return get_runtime_dir(root) / "python"
...
def _install_safe_python_generation(
    ...
    *,
    ...
    runtime_root = get_runtime_dir(project_root)
...
def _refresh_managed_uv_catalog(uv_bin: str) -> bool:
    """Re-provision the pinned uv to refresh its Python catalog.
    ...
    pin bump.  *uv_bin* is always the pinned binary (the caller resolved it
    through ``installation.uv``); there is no foreign-uv case.
...
def repair_vulnerable_runtime(
    *,
    ...
    uv is resolved internally — the pinned binary via ``installation.uv``.
    There is no foreign-uv path: the pin table names exactly one uv, and a
    repair that ran on someone else's uv would provision an interpreter
    outside the catalog the pins promise.
    """
    root = Path(project_root) if project_root is not None else _PROJECT_ROOT
    ...
    if not uv_bin:
        return RuntimeRepairResult(
            "skipped", "pinned uv unavailable (provisioning failed)"
        )
    ...
    runtime_root = get_runtime_dir(root)
