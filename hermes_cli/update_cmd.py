...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        if stable_tag is not None:
            # Stable channel: the target is the release tag, not a branch.
            # No branch switching — the checkout fast-forwards (or resets)
            # to the tag's commit.
        ...
        try:
            ...
            if pull_result.returncode != 0:
                ...
                if stable_tag is None and _cur_branch and _cur_branch != branch:
                    print(
        ...
        # Note: no cua-driver refresh step here — the driver is a pinned
        # managed tool; provision_runtimes carries pin bumps (post_update).

        # Fleet version verification (#88654): after the phase's gateway
        # restart, prove every runtime actually runs post-update code, and
        # finalize the update receipt with the fleet snapshot.
        gateway_fleet_restart_incomplete = bool(phase_rc)
