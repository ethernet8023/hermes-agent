    ...
    with store.install_lock():
        ...
        if not store.published(entry_name):
            if sha256 is None:
                raise InstallError(
                    package.name,
                    f"no hash for {target} in the lockfile",
            ...
            with store.scratch() as scratch:
                ...
                try:
                    ...
                    raise InstallError(package.name, f"download failed: {e}") from e
...
def ensure(name: str, *, base_env: Optional[dict] = None) -> Runner:
...
def uv(*, venv=None, realize: bool = True):
    """TRANSITIONAL (dies with the update/repair collapse, plan step 4):
    (uv path, sanitized env) for the update/repair call sites that still
    drive uv themselves. Their replacement is the venv package —
    python-dep syncing belongs inside pm install, not at call sites."""
