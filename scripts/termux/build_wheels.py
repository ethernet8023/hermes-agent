        ...
        if name in ("psutil", "uvloop"):
            ...
            with tempfile.TemporaryDirectory(prefix=f"hermes-build-{name}-") as tmp:
                ...
                if name == "uvloop":
                    ...
                    # expects libuv configured before setup.py runs it.
                    # Bootstrap in place (autotools apt-installed).
                    ...
                    if libuv.is_dir():
                        ...
                        if proc.returncode == 0 and (libuv / "configure").is_file():
                            # The container cannot create /bin/sh (unprivileged
                            # user, read-only /); rewrite the shebang to the
                            # container's own sh instead.
                            cfg = libuv / "configure"
                            text = cfg.read_text(encoding="utf-8", errors="replace")
                            if text.startswith("#!/bin/sh"):
                                cfg.write_text(text.replace("#!/bin/sh", f"#!{os.environ['PREFIX']}/bin/sh", 1), encoding="utf-8")
                            cfg.chmod(0o755)
