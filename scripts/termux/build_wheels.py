...
def write_reqs_file(resolved: Path, reqs: Path) -> None:
    """name==spec[; marker] lines for a pip/uv -r file: markers stay
    attached so the INSTALLER evaluates them on the target platform
    (win32-marker deps resolve-skip on bionic, not fail).
    """
    out = []
