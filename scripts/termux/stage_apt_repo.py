...
def stage(pool_dir: Path, out_dir: Path, suite: str, gpg_key_file: Path | None) -> int:
    ...
    for deb in debs:
        ...
        stanza = [
            ...
            f"Architecture: {arch}",
            f"Maintainer: {fields.get('Maintainer', 'Hermes Agent <noreply@nousresearch.com>')}",
            f"Installed-Size: {fields.get('Installed-Size', '0')}",
            f"Description: {fields.get('Description', 'Hermes Agent')}",
            f"Filename: {filename}",
            f"Size: {size}",
            f"SHA256: {sha256}",
            "",
    ...
    def stanza_key(s: str):
        ...
        return (pkg, deb_version_key(ver))

    stanzas.sort(key=stanza_key)
    packages_text = "\n".join(stanzas) + ("\n" if stanzas else "")
    ...
    release_fields = [
        f"Origin: Hermes Agent",
        f"Label: hermes-agent",
        f"Suite: {suite}",
        f"Codename: {suite}",
    ...
    for name in ("Packages", "Packages.gz"):
        ...
        size = p.stat().st_size
        checksums.append(f" {hashlib.sha256(p.read_bytes()).hexdigest()} {size:8d} {rel}")
        sha512.append(f" {hashlib.sha512(p.read_bytes()).hexdigest()} {size:8d} {rel}")
    release = "\n".join(release_fields) + "\n"
    release += "\nSHA256:\n" + "\n".join(checksums) + "\n"
    release += "\nSHA512:\n" + "\n".join(sha512) + "\n"
