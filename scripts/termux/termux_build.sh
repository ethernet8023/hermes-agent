...
if [ "${1:-}" = "--in-container" ]; then
    ...
    echo "DEBUG: PAYLOAD_ROOT=$PAYLOAD_ROOT" >&2
    echo "DEBUG: top of mount:" >&2
    ls -la "$PAYLOAD_ROOT" >&2 2>&1 | head -12 >&2 || true
    echo "DEBUG: python dir if any:" >&2
    ls -la "$PAYLOAD_ROOT/python" >&2 || true
