...
if [ "${1:-}" = "--in-container" ]; then
    ...
    if ! command -v clang >/dev/null 2>&1; then
        ...
        # Pin the OFFICIAL mirror: auto-picked third-party mirrors desync
        # their pool against the index (live: a 404 on libexpat from a
        # stale mirror mid-run). Deterministic builds get deterministic
        # sources. Retry the update once for propagation windows.
