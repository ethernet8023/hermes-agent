...
if [ "${1:-}" = "--in-container" ]; then
    ...
    # The image is a BARE termux bootstrap: no python, no clang. The build
    # toolchain comes from termux's own apt repos (adybag's proven prelude).
    # The USER machine never does any of this -- this is build-runner
    # provisioning inside the pinned container.
    if ! command -v python >/dev/null 2>&1; then
        log "Provisioning the container build toolchain (termux apt)"
        export DEBIAN_FRONTEND=noninteractive
        pkg update \
            || fail "pkg update failed in the container"
        pkg install -y python clang rust make patchelf binutils \
    ...
    # (there is no `rust` binary); python provides python3.
    for tool in python python3 clang rustc cargo make; do
    ...
    python -m pip install "${TOOLCHAIN_PINS[@]}" \
        || fail "toolchain pin enforcement failed"
    log "Building the android wheel set from sdist (bionic)"
    python "$HERE/build_wheels.py" \
...
# bionic, and only the container's termux python + clang/rust can make them.
log "Building the wheelhouse inside the pinned container"
...
docker run --rm --platform linux/arm64 \
    ...
    "$IMAGE" bash /repo/scripts/termux/termux_build.sh \
        --in-container "$C_RESOLVED" "$C_BUILD_SET" "$C_WHEELHOUSE" \
