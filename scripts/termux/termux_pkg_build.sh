...
docker run --rm --platform linux/arm64 \
  ...
  "$IMAGE" bash -c '
    ...
    export MAKEFLAGS=-j1
    # Invoke build-package.sh THROUGH bash: the container is bionic and
    # has no /bin/bash (bash lives at the termux $PREFIX, on PATH), so
    # executing scripts directly dies on their shebang. run-docker.sh is
    # the HOST-side docker wrapper and must never run inside the container.
    # TERMUX_PREFIX is deliberately NOT overridden: the build installs
    # into the bootstrap prefix, the copy-out below stages the tree into
    # /payload, and baked $PREFIX paths are correct on-device because
    # the Termux $PREFIX is contractual.
    bash ./build-package.sh -a aarch64 -f '"$RECIPE"'
