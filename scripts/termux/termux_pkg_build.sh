...
# pins.json keys the runtimes as python | node — the recipe name differs for
# node (nodejs-lts). Map explicitly so the pin lookup can never silently
# return null and curl a bogus "null" host.
case "$RECIPE" in
  python)     PINS_KEY=python ;;
  nodejs-lts) PINS_KEY=node ;;
  *) exit 1 ;;
esac
...
STAMP_WANT="$RECIPE $(jq -r --arg k "$PINS_KEY" '.[$k].debVersion' "$PINS")"
...
DEB_URL=$(jq -r --arg k "$PINS_KEY" '.[$k].debUrl' "$PINS")
DEB_SHA=$(jq -r --arg k "$PINS_KEY" '.[$k].debSha256' "$PINS")
[ -n "$DEB_URL" ] && [ "$DEB_URL" != "null" ]   || { echo "termux_pkg_build($RECIPE): pins.json has no debUrl under key '$PINS_KEY'" >&2; exit 1; }
[ -n "$DEB_SHA" ] && [ "$DEB_SHA" != "null" ]   || { echo "termux_pkg_build($RECIPE): pins.json has no debSha256 under key '$PINS_KEY'" >&2; exit 1; }
...
WANT=$(jq -r --arg k "$PINS_KEY" '.[$k].debVersion' "$PINS")
[ "$GOT" = "$WANT" ] \
