...
def _install_root_is_tmp(tmp_path, monkeypatch):
    """runtime_repair resolves through get_runtime_dir() -> install root.
    Point the install root at tmp_path so every test's runtime artifacts
    land in an isolated .hermes-runtime (hermes-home lifetime split, 2.5)."""
    monkeypatch.setenv("HERMES_INSTALL_ROOT", str(tmp_path))


@pytest.fixture(autouse=True)
def _pinned_uv_resolves(monkeypatch):
    """repair_vulnerable_runtime() resolves uv itself via
    installation.uv.ensure_uv; pin it so no test provisions for real."""
    monkeypatch.setattr("installation.uv.ensure_uv", lambda: "uv")
...
class TestRefreshManagedUvCatalog:
    ...
    def test_version_change_reports_true(self, monkeypatch):
        import hermes_cli.runtime_repair as runtime_repair
        from installation.provisioner import ToolResult

        versions = iter(["uv 0.1.0", "uv 0.2.0"])
        monkeypatch.setattr(
            runtime_repair, "_uv_version_string", lambda _uv: next(versions)
        )
        monkeypatch.setattr(
            "installation.provisioner.provision_tool",
            lambda name: ToolResult("uv", "downloaded", version="0.2.0"),
        )
        assert runtime_repair._refresh_managed_uv_catalog("uv") is True

    def test_unchanged_version_reports_false(self, monkeypatch):
        """When the pin did not move, a retry would resolve identically."""
        import hermes_cli.runtime_repair as runtime_repair
        from installation.provisioner import ToolResult

        monkeypatch.setattr(
            runtime_repair, "_uv_version_string", lambda _uv: "uv 0.1.0"
        )
        monkeypatch.setattr(
            "installation.provisioner.provision_tool",
            lambda name: ToolResult("uv", "kept", version="0.1.0"),
        )
        assert runtime_repair._refresh_managed_uv_catalog("uv") is False

    def test_provisioner_failure_reports_false(self, monkeypatch):
        import hermes_cli.runtime_repair as runtime_repair
        ...
        monkeypatch.setattr("installation.provisioner.provision_tool", boom)
        assert runtime_repair._refresh_managed_uv_catalog("uv") is False
...
class TestRepairRetriesAfterUvRefresh:
    def _run_repair(self, tmp_path, *, refresh_result, second_attempt):
        ...
        with patch(
             ...
             patch(
                 "hermes_cli.runtime_repair._refresh_managed_uv_catalog",
