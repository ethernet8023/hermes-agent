...
The chromium CfT archives carry a macOS .framework whose layout is five
symlinks (Versions/Current, Resources, ...). zipfile.extract() writes a
symlink entry's TARGET PATH out as a regular file, which destroys the
bundle: codesign then refuses even a fresh re-sign with "embedded
framework contains modified or invalid version" (reproduced on a real
mac against chromium pin 1208). So _extract recreates link entries with
os.symlink — but only the safe ones.
...
from installation.provisioner import _extract
...
def _add_file(zf: zipfile.ZipFile, member: str, data: bytes = b"x", mode: int = 0o644) -> None:
    info = zipfile.ZipInfo(member)
...
posix_only = pytest.mark.skipif(
    sys.platform == "win32", reason="symlink recreation degrades to files on win32"
)
...
def test_framework_shaped_links_round_trip(tmp_path: Path) -> None:
    ...
    dest = tmp_path / "out"
    _extract(_write_zip(tmp_path / "a.zip", build), dest)
...
@posix_only
def test_file_written_through_earlier_link_entry_cannot_escape(tmp_path: Path) -> None:
    ...
    path and nothing is written outside dest."""
