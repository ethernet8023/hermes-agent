...
can ask for were collected by walking every shipped revision of the
update flow (scripts/audit-old-updater-imports.py) and frozen into
    ...
    def test_has_the_load_bearing_names(self):
        ...
        for anchor in (
            ...
            assert anchor in bare, (
                f"{anchor} missing from the frozen surface — regenerate "
                f"on a FULL clone (a shallow walk shrinks the surface)."
            )

    def test_walk_was_not_shallow(self):
        stats = _load_surface()["stats"]
        assert stats["commits"] >= 60, (
            f"frozen surface built from only {stats['commits']} commits — "
            f"that smells like a shallow clone; regenerate on a full one."
        )
