...
| Give the agent an identity | `hermesHomeDocuments."SOUL.md"` | `"You are a terse ops assistant."` |
...
`hermesHomeDocuments` installs into **`HERMES_HOME`**. Hermes reads the identity file and the memory files of the agent from that directory. `SOUL.md` and `memories/` work only from there. A `SOUL.md` in `documents` makes a workspace file, and Hermes does not load it as the identity:

```nix
{
  services.hermes-agent.hermesHomeDocuments = {
...
| Option | Type | Default | Description |
|---|---|---|---|
| `documents` | `attrsOf (either str path)` | `{}` | Workspace files. Each key is a path relative to `workingDirectory`. Each value is a string or a path. |
| `hermesHomeDocuments` | `attrsOf (either str path)` | `{}` | Files that go into `HERMES_HOME`. `SOUL.md` and `memories/` must be here, or Hermes does not load them. |
...
├── .hermes/                         # HERMES_HOME
│   ├── SOUL.md                      # from hermesHomeDocuments: the agent identity
...
├── SOUL.md                          # from hermesHomeDocuments
